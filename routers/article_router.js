...
router.get('/pageList', async function (req, res, next) {
  ...
  if (ordername && orderby) {
    orderData.push([ordername, orderby]);
  }
...
router.post('/add', async function (req, res, next) {
  ...
  const { title, img, type_id, is_comment, status, content, click, tags } =
    req.body;
...
router.put('/update', async function (req, res, next) {
  const { id, title, img, is_comment, status, content, click, tags, type_id } =
    req.body;
  let update_article = await Article.findByPk(id);
  await update_article.update({
    title,
    img,
    is_comment,
    status,
